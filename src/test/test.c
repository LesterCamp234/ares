...
void test_unknown_opcode(void) {
    ...
    TEST_ASSERT_EQUAL_STRING(g->error, "Unknown opcode");
}

void test_unterminated_instruction(void) {
    assemble_line("addi x1, x2 ");
    TEST_ASSERT_EQUAL_STRING(g->error, "Expected ,");
}
...
void test_number_parsing_i64_overflow(void) {
    ...
    TEST_ASSERT_EQUAL_STRING(g->error, "Invalid word");
}
...
void test_sw_immediate_negative(void) {
    ...
    TEST_ASSERT_EQUAL_INT(g->text->contents.len, 4);
    u32 word = LOAD(g, g->text->base, 4, &err);
    TEST_ASSERT_EQUAL_INT(word, 0xfe112fa3);
}

void test_instruction_trailing_comma(void) {
    assemble_line("addi x1, x2, 300,");
    TEST_ASSERT_EQUAL_STRING(g->error, "Expected newline");
}
...
void test_add_invalid_reg(void) {
    ...
    TEST_ASSERT_EQUAL_STRING(g->error, "Invalid rs2");
}

void test_add_invalid_reg_2(void) {
    assemble_line("add xb, xc, xa");
    TEST_ASSERT_EQUAL_STRING(g->error, "Invalid rd");
}
...
void test_shift_invalid_imm_1(void) {
    ...
    TEST_ASSERT_EQUAL_STRING(g->error, "Invalid shift immediate");
}
...
void test_add_invalid_reg_4(void) {
    ...
    TEST_ASSERT_EQUAL_STRING(g->error, "Invalid rs1");
}

void test_case_insensitivity(void) {
    assemble_line("ADDI X1, X2, 0X41");
    TEST_ASSERT_EQUAL_STRING(g->error, NULL);
}
...
void test_parse_directives_nums(void) {
    bool err;
    assemble_line(".data\nvar: .WORD 5");
    TEST_ASSERT_EQUAL_INT(g->data->contents.len, 4);
    u32 word = LOAD(g, g->data->base, 4, &err);
    TEST_ASSERT_EQUAL_INT(word, 5);
    free_runtime(g);

    assemble_line(".DATA\nvar: .HALF 5");
    TEST_ASSERT_EQUAL_INT(g->data->contents.len, 2);
    u16 half = LOAD(g, g->data->base, 2, &err);
    TEST_ASSERT_EQUAL_INT(half, 5);
    free_runtime(g);

    assemble_line(".data\nvar: .byte 5");
    TEST_ASSERT_EQUAL_INT(g->data->contents.len, 1);
    u8 byte = LOAD(g, g->data->base, 1, &err);
...
void test_parse_directives_nums_oob(void) {
    assemble_line(".data\nvar: .half 0x10000");
    TEST_ASSERT_EQUAL_STRING(g->error, "Out of bounds half");
    free_runtime(g);
    ...
    assemble_line(".data\nvar: .byte 0x100");
    TEST_ASSERT_EQUAL_STRING(g->error, "Out of bounds byte");
    free_runtime(g);
...
void test_jalr_1(void) {
    ...
    TEST_ASSERT_EQUAL(0x000200e7, LOAD(g, g->text->base, 4, &err));
}

void test_jalr_2(void) {
    bool err = false;
    assemble_line("jalr x4, x5");
    TEST_ASSERT_EQUAL(0x00028267, LOAD(g, g->text->base, 4, &err));
}

void test_jalr_3(void) {
    bool err = false;
    assemble_line("jalr x4, x5, 6");
    TEST_ASSERT_EQUAL(0x00628267, LOAD(g, g->text->base, 4, &err));
}
...
void test_data_label_1(void) {
    bool err = false;
    assemble_line(".data\nhi: .word label\n.text\nlabel:\nnop");
    TEST_ASSERT_EQUAL(TEXT_BASE, LOAD(g, g->data->base, 4, &err));
}
...
void test_data_label_4(void) {
    ...
    assemble_line(".data\nhi: .word label, 2\n.text\nlabel:\nnop");
    TEST_ASSERT_EQUAL(TEXT_BASE, LOAD(g, g->data->base, 4, &err));
    TEST_ASSERT_EQUAL(2, LOAD(g, g->data->base + 4, 4, &err));
}
...
void test_branch_forward_out_of_range(void) {
    ...
    strcat(buf, "far:\nnop");
    assemble_line(buf);
    TEST_ASSERT_EQUAL_STRING(g->error, "Branch immediate too large");
}
...
void test_globl_without_definition(void) {
    ...
    u32 addr;
    Section *sec;
    TEST_ASSERT_FALSE(resolve_symbol(g, "foo", 3, true, &addr, &sec));
}

void test_parse_directives_str(void) {
    assemble_line(".data\nstr: .ASCII \"hi\", \"hi\"");
    TEST_ASSERT_EQUAL_STR("hihi", g->data->contents.buf, g->data->contents.len);
    free_runtime(g);

    assemble_line(".data\nstr: .string \"hi\"");
    TEST_ASSERT_EQUAL_INT(g->data->contents.len, 3);
    TEST_ASSERT_EQUAL_CHAR_ARRAY("hi\0", g->data->contents.buf,
                                 g->data->contents.len);
    free_runtime(g);
}
...
void test_resolve_text_symbol_found(void) {
    ...
    Section *sec;
    bool found =
        resolve_symbol(g, "mylabel", strlen("mylabel"), false, &addr, &sec);
    TEST_ASSERT_TRUE(found);
    TEST_ASSERT_EQUAL(sec, g->text);
    TEST_ASSERT_TRUE(addr >= g->text->base && addr < g->text->limit);
}
...
void test_resolve_data_symbol_found(void) {
    ...
    bool found =
        resolve_symbol(g, "mylabel", strlen("mylabel"), false, &addr, &sec);
    TEST_ASSERT_TRUE(found);
    TEST_ASSERT_EQUAL(sec, g->data);
    TEST_ASSERT_TRUE(addr >= g->data->base && addr < g->data->limit);
}
...
void test_pc_to_label_r2(void) {
    ...
    LabelData *ret = NULL;
    u32 off = 0;
    bool result = pc_to_label_r(g, g->text->base, &ret, &off);
...
void test_pc_to_label_r_no_label(void) {
    ...
    LabelData *ret = NULL;
    u32 off = 0;
    bool result = pc_to_label_r(g, 0xdeadbeef, &ret, &off);
...
void test_fixup(void) {
    ...
    bool err;
    TEST_ASSERT_EQUAL_INT(LOAD(g, g->text->base, 4, &err), 0x0040006f);
...
void test_globl_empty(void) {
    assemble_line(".globl\n");
    TEST_ASSERT_EQUAL_STRING(g->error, "Expected identifier after .globl");
}

void test_nolabel(void) {
    assemble_line("j ");
    TEST_ASSERT_EQUAL_STRING(g->error, "No label");
}

void test_nonglobal_start(void) {
    assemble_line("_start: ");
    TEST_ASSERT_EQUAL_STRING(g->error, "_start defined, but without .globl");
}

void test_start_in_data(void) {
    assemble_line(".globl _start\n.data\n_start: ");
    TEST_ASSERT_EQUAL_STRING(g->error, "_start not in .text section");
}
...
void test_word_list(void) {
    assemble_line(".data\n.word 1, 2, 3");
    TEST_ASSERT_NULL(g->error);
    TEST_ASSERT_EQUAL_INT(12, g->data->contents.len);
    bool err;
    TEST_ASSERT_EQUAL_INT(1, LOAD(g, g->data->base, 4, &err));
    TEST_ASSERT_EQUAL_INT(2, LOAD(g, g->data->base + 4, 4, &err));
...
void test_ascii_no_null_terminator(void) {
    assemble_line(".data\n.ascii \"hi\"");
    TEST_ASSERT_NULL(g->error);
    TEST_ASSERT_EQUAL_INT(2, g->data->contents.len);
}

void test_asciz_null_terminator(void) {
    assemble_line(".data\n.asciz \"hi\"");
    TEST_ASSERT_NULL(g->error);
    TEST_ASSERT_EQUAL_INT(3, g->data->contents.len);
    TEST_ASSERT_EQUAL_CHAR('\0', g->data->contents.buf[2]);
}
...
void test_label_address_increments(void) {
    ...
    Section *sec;
    resolve_symbol(g, "foo", 3, false, &foo_addr, &sec);
    resolve_symbol(g, "bar", 3, false, &bar_addr, &sec);
...
void test_section_switch(void) {
    assemble_line(
        ".data\nfoo: .word 99\n.text\nadd x0,x0,x0\n.data\nbar: .word 88");
    TEST_ASSERT_NULL(g->error);
    TEST_ASSERT_EQUAL_INT(8, g->data->contents.len);
    TEST_ASSERT_EQUAL_INT(4, g->text->contents.len);
}
...
void test_resolve_symbol_not_found(void) {
    ...
    u32 addr;
    Section *sec;
    TEST_ASSERT_FALSE(resolve_symbol(g, "foo", 5, false, &addr, &sec));
}
...
void test_label_at_end_of_file(void) {
    ...
    u32 addr;
    Section *sec;
    TEST_ASSERT_TRUE(resolve_symbol(g, "foo", 3, false, &addr, &sec));
}

void test_unknown_section(void) {
    assemble_line(".section .foo");
    TEST_ASSERT_EQUAL_STRING(g->error, "Section not found");
}

void test_la_deferred(void) {
    bool err;
    assemble_line("la t0, hi\nhi: nop");
    TEST_ASSERT_EQUAL_INT(LOAD(g, g->text->base, 4, &err), 0x00000297);
    TEST_ASSERT_EQUAL_INT(LOAD(g, g->text->base + 4, 4, &err), 0x00828293);
}
...
foo:\n\
    ...
    lw    a1, 0(a1)\n\
");
    TEST_ASSERT_EQUAL_INT(g->text->by_linenum.len, 7 * 4);
}
...
void test_hi_lo_int(void) {
    ...
    expected_lui = LOAD(g, g->text->base, 4, &err);
    expected_addi = LOAD(g, g->text->base + 4, 4, &err);
    free_runtime(g);
    g->error = NULL;
    assemble_line("lui a0, %hi(123456)\naddi a0, a0, %lo(123456)\n");
    TEST_ASSERT_EQUAL_INT(expected_lui, LOAD(g, g->text->base, 4, &err));
    TEST_ASSERT_EQUAL_INT(expected_addi, LOAD(g, g->text->base + 4, 4, &err));
}
...
void build_and_run(const char *txt) {
    u32 addr;
    assemble(g, txt, strlen(txt), false);
    TEST_ASSERT_EQUAL_STRING(g->error, NULL);
    if (resolve_symbol(g, "_start", strlen("_start"), true, &addr, NULL))
        g->pc = addr;
    while (!g->exited) {
        emulate(g);
        if (g->runtime_error_type != ERROR_NONE) break;
    }
}
void check_pc_at_label(const char *label) {
    u32 addr;
    TEST_ASSERT_TRUE(
        resolve_symbol(g, label, strlen(label), false, &addr, NULL));
    TEST_ASSERT_EQUAL(g->pc, addr);
}
...
E:  .word 0x00003023  \n\
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_UNHANDLED_INSN);
    check_pc_at_label("E");
}
...
E:  addi a3, a3, 1 \n\
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_CALLSAN_CALL_CLOBBERED);
    TEST_ASSERT_EQUAL(g->runtime_error_params[0], REG_A3);
    check_pc_at_label("E");
}
...
E:  addi a3, a3, 1 \n\
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_CALLSAN_CANTREAD);
    TEST_ASSERT_EQUAL(g->runtime_error_params[0], REG_A3);
    check_pc_at_label("E");
}
...
_start:         \n\
    jal fn      \n\
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_CALLSAN_NOT_SAVED);
    TEST_ASSERT_EQUAL(g->runtime_error_params[0], REG_S1);
    check_pc_at_label("E");
}
...
_start:              \n\
    jal fn           \n\
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_CALLSAN_RA_MISMATCH);
    check_pc_at_label("E");
}
...
_start:              \n\
    ...
    ecall            \n\
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_CALLSAN_SP_MISMATCH);
    check_pc_at_label("E");
}
...
E:  ret              \n\
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_CALLSAN_RET_EMPTY);
    check_pc_at_label("E");
}
...
_start:             \n\
    jal fn          \n\
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_CALLSAN_LOAD_STACK);
    check_pc_at_label("E");
}
...
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_CALLSAN_CALL_CLOBBERED);
    TEST_ASSERT_EQUAL(g->runtime_error_params[0], REG_T0);
    check_pc_at_label("E");
}
...
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_CALLSAN_CALL_CLOBBERED);
    TEST_ASSERT_EQUAL(g->runtime_error_params[0], REG_A2);
    check_pc_at_label("E");
}
...
_start:\n\
    ...
    ecall\n\
");
    TEST_ASSERT_EQUAL_UINT32(5U, g->regs[REG_A0]);
    TEST_ASSERT_EQUAL_UINT32((u32)-3, g->regs[REG_A1]);
}
...
_start:\n\
    ...
    ecall\n\
");
    TEST_ASSERT_EQUAL_UINT32(0x1234U, g->regs[REG_A1]);
}
...
");
    TEST_ASSERT_EQUAL_UINT32(0x1234U, g->regs[REG_A1]);
    TEST_ASSERT_EQUAL_UINT32(0x123U, g->regs[REG_A0]);
}
...
.data\n\
    dummy: .byte 0\n\
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_STORE);
}

void test_load_store_api(void) {
    assemble_line(".data\nvar: .word 0");
    bool err = false;
    STORE(g, g->data->base, 0xDEADBEEFu, 4, &err);
    TEST_ASSERT_FALSE(err);
    u32 val = LOAD(g, g->data->base, 4, &err);
    TEST_ASSERT_FALSE(err);
...
void test_kernel_memory_protection(void) {
    ...
    u32 val = LOAD(g, g->kernel_data->base, 4, &err);
    TEST_ASSERT_TRUE(err);

    // but kernel mode should
    emulator_enter_kernel(g);
    err = false;
    val = LOAD(g, g->kernel_data->base, 4, &err);
    TEST_ASSERT_FALSE(err);
...
void step(void) {
    emulate(g);
    TEST_ASSERT_EQUAL(ERROR_NONE, g->runtime_error_type);
}
...
";
    assemble(g, prog, strlen(prog), false);
    TEST_ASSERT_EQUAL_STRING(g->error, NULL);
    TEST_ASSERT_TRUE(g->kernel_text->contents.len > 0);
    TEST_ASSERT_TRUE(g->text->contents.len > 0);
    g->pc = g->kernel_text->base;

    g->csr[CSR_STVEC] = g->kernel_text->base;

    u32 addr;
    TEST_ASSERT_TRUE(
        resolve_symbol(g, "_start", strlen("_start"), true, &addr, NULL));
    g->pc = addr;

    emulator_leave_kernel(g);
    emulate(g);  // one single instruction
    TEST_ASSERT_EQUAL(g->pc, g->csr[CSR_STVEC]);
    TEST_ASSERT_EQUAL(addr, g->csr[CSR_SEPC]);
}
...
void test_emulator_interrupt_set_pending(void) {
    ...
    g->csr[CSR_MIP] = 0;
    g->csr[CSR_STVEC] = 0xAABB00;
    emulator_interrupt_set_pending(g,
                                   CAUSE_SUPERVISOR_TIMER & ~CAUSE_INTERRUPT);
    TEST_ASSERT_TRUE(g->csr[CSR_MIP] &
                     (1u << (CAUSE_SUPERVISOR_TIMER & ~CAUSE_INTERRUPT)));
    emulate(g);
    TEST_ASSERT_EQUAL_UINT32(g->pc, g->csr[CSR_STVEC]);
    TEST_ASSERT_EQUAL_UINT32(g->text->base, g->csr[CSR_SEPC]);
    TEST_ASSERT_EQUAL_UINT32(CAUSE_SUPERVISOR_TIMER, g->csr[CSR_SCAUSE]);
}
...
void test_sret_returns_to_sepc(void) {
    ...
    TEST_ASSERT_EQUAL_STRING(g->error, NULL);
    u32 addr;
    TEST_ASSERT_TRUE(resolve_symbol(g, "return_target", strlen("return_target"),
                                    false, &addr, NULL));
    g->csr[CSR_SEPC] = addr;
    g->pc = g->kernel_text->base;
    emulator_enter_kernel(g);
    step();
    TEST_ASSERT_EQUAL(g->pc, g->csr[CSR_SEPC]);
}
...
void test_jump_to_exception_delegation(void) {
    ...
    g->csr[CSR_STVEC] = 0xAABB00u;

    step();

    TEST_ASSERT_EQUAL_UINT32(g->text->base, g->csr[CSR_SEPC]);
    TEST_ASSERT_EQUAL_UINT32(CAUSE_U_ECALL, g->csr[CSR_SCAUSE]);
    TEST_ASSERT_EQUAL_UINT32(g->csr[CSR_STVEC], g->pc);
}
...
";
    ...
    u32 vector_handlers;
    TEST_ASSERT_TRUE(resolve_symbol(g, "vector_handlers",
                                    strlen("vector_handlers"), false,
                                    &vector_handlers, NULL));

    g->csr[CSR_STVEC] = vector_handlers | 1;
...
";
    ...
    step();
    step();
    TEST_ASSERT(g->csr[CSR_MSTATUS] != -1u);
}
...
";
    ...
    step();
    step();
    TEST_ASSERT_TRUE(g->csr[CSR_MSTATUS] & STATUS_SPP);
}
...
";
    ...
    step();
    step();
    TEST_ASSERT_FALSE(g->csr[CSR_MSTATUS] & STATUS_SIE);
    TEST_ASSERT_TRUE(g->csr[CSR_MSTATUS] & STATUS_SPIE);
}

void test_rvc_insn_size(void) {
    assemble_line("c.nop\nc.nop\n");
    TEST_ASSERT_EQUAL_INT(g->text->by_linenum.len, 2 + 2);
}

void test_rvc_mixed_width(void) {
    assemble_line("c.nop\nnop\nc.nop\n");
    TEST_ASSERT_EQUAL_INT(g->text->by_linenum.len, 2 + 4 + 2);
}
...
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_NONE);
    TEST_ASSERT_EQUAL_UINT32(15U, g->regs[REG_A0]);
}
...
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_NONE);
    TEST_ASSERT_EQUAL_UINT32(3, g->regs[REG_A0]);
}
...
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_NONE);
    TEST_ASSERT_EQUAL_UINT32(4, g->regs[REG_A2]);
}
...
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_NONE);
    TEST_ASSERT_EQUAL_UINT32(0U, g->regs[REG_A0]);
}
...
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_NONE);
    TEST_ASSERT_EQUAL_UINT32(7U, g->regs[REG_A0]);
}
...
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_NONE);
    TEST_ASSERT_EQUAL_UINT32(16U, g->regs[REG_A0]);
}
...
");
    TEST_ASSERT_EQUAL(g->runtime_error_type, ERROR_NONE);
    TEST_ASSERT_EQUAL_UINT32(1234, g->regs[REG_A0]);
}
